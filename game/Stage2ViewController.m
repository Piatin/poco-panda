...
@interface Stage2ViewController ()<UIActionSheetDelegate>
...
- (void)viewDidLoad
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        if ([[UIScreen mainScreen] respondsToSelector: @selector(scale)]) {
            ...
            if(result.height == 960){
                // iPhone4 or 4S
                NSLog(@"iphone 4, 4s imgview");
                panda = [[UIImageView alloc] initWithFrame:CGRectMake(-80, 85, 450, 320)];
                
            }
            if(result.height == 1136){
                // iPhone5
                NSLog(@"iphone 5 imgview");
                panda = [[UIImageView alloc] initWithFrame:CGRectMake(-80, 140, 450, 320)];
    ...
    // アニメーション用画像をセット
    panda.animationImages = imageList;
    
    
    // アニメーションの速度
    panda.animationDuration = 10.0;
    
    // アニメーションのリピート回数
    panda.animationRepeatCount = 0;
    
    [panda startAnimating];
...
-(void)down{
    ...
    if(time <= 0){
        ...
        [self performSegueWithIdentifier:@"stage2ToTopSegue" sender:self];
        [bgmGam stop];
...
- (void) handleSwipeUpGesture1:(UISwipeGestureRecognizer *)sender {
    ...
    if(ansGazou ==0 && time>=0){
        ...
        [ten setInteger:sum forKey:@"index2"];
        
        [ten synchronize];
        
        NSUserDefaults *me = [NSUserDefaults standardUserDefaults];
        
        [me setInteger:time forKey:@"time2"];
        ...
        animationView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 568, 320, 568)];
        animationView.image = [UIImage imageNamed:@"gameclear.png"];
        [self.view addSubview:animationView];
        ...
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
        {
            ...
            if ([[UIScreen mainScreen] respondsToSelector: @selector(scale)]) {
                ...
                if(result.height == 960){
                    // iPhone4 or 4S
                    NSLog(@"iphone 4, 4s imgview");
                    compPanda = [[UIImageView alloc] initWithFrame:CGRectMake(-47, 100, 400, 400)];
                }
                if(result.height == 1136){
                    // iPhone5
                    NSLog(@"iphone 5 imgview");
                    compPanda = [[UIImageView alloc] initWithFrame:CGRectMake(-80, 120, 450, 450)];
                }
            }
        ...
        for (NSInteger i = 1; i < 5; i++) {
            NSString *imagePath = [NSString stringWithFormat:@"comp%02d.png", i];
            UIImage *img = [UIImage imageNamed:imagePath];
            [imageList addObject:img];
        }
        
        [self.view addSubview:compPanda];
        // アニメーション用画像をセット
        compPanda.animationImages = imageList;
        
        
        // アニメーションの速度
        compPanda.animationDuration = 0.7;
        
        // アニメーションのリピート回数
        compPanda.animationRepeatCount = 0;
        
        [compPanda startAnimating];
        [self.view bringSubviewToFront:compPanda];
        ...
        [self performSegueWithIdentifier:@"stage2ToResultSegue" sender:self];
        [bgmGam stop];
...
- (void) handleSwipeUpGesture2:(UISwipeGestureRecognizer *)sender {
    ...
    if(ansGazou ==1 && time>=0){
        ...
        [ti setInteger:time forKey:@"time2"];
